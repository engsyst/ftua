...
public class ClubSettingsPanel extends Composite implements ClubUpdater {
	...
	private void drawContent() {
		...
		for (ClubSettingViewData cvd : clubs) {
			...
			if (cvd.getInner() != null) {
				t.setWidget(row, 2, new ScheduleClubTitleLabel(cvd.getInner()));
	...
	private ClickHandler checkHandler = new ClickHandler() {
		...
		public void onClick(ClickEvent event) {
			...
			try {
				setClubIndepended(
						Long.parseLong(cb.getElement().getId().split("-")[1]),
						cb.getValue());
			} catch (Exception e) {
			}
		}
	...
	private void removeClub(Club club) {
		AppState.startSettingsService.removeClub(club.getClubId(),
	...
	protected void importClub(Club innerClub) {
		AppState.startSettingsService.importClub(innerClub,
				new AsyncCallback<Club>() {
	...
	private void getAllClubs() {
		AppState.startSettingsService
				.getAllClubs(new AsyncCallback<List<ClubSettingViewData>>() {
	...
	private void setClubIndepended(long id, boolean isIndepended) {
		AppState.startSettingsService.setClubIndependent(id, isIndepended,
				new AsyncCallback<Club>() {
					...
					public void onSuccess(Club result) {
						...
						CheckBox cb = (CheckBox) t.getWidget(clubs.get(idx)
								.getRow(), 3);
