...
public class ScheduleManagerServiceImpl extends RemoteServiceServlet implements
	...
	public CategorySettingsData getCategorySettingsData()
		...
		try {
			employeeList = employeeDAO.getAllAdminScheduleEmployees();
		} catch (DAOException e) {
			throw new IllegalArgumentException(e);
		}
	...
	public List<Employee> getScheduleEmployees()
			throws IllegalArgumentException {
		List<Employee> scheduleEmployees;
		try {
			scheduleEmployees = employeeDAO.getAllAdminScheduleEmployees();
		} catch (DAOException e) {
			throw new IllegalArgumentException(e);
		}
...
//				GenFlags.WEEKEND_AFTER_MAX_HOURS);
		...
		try {
			sEmps.addAll(employeeDAO.getAllAdminScheduleEmployees());
		} catch (DAOException e) {
	...
	public void removeSchedule(long id) throws IllegalArgumentException,
		...
		log.info(logMessage("Удаление", "график работ c Id", Long.toString(id)));
	}
