...
public class ExcelScheduleUtil {
	 ...
	 *         conversion fails.
	 */
	public static byte[] scheduleAllToExcelConvert(Schedule schedule)
			throws NullPointerException {

		if (schedule == null) {
			throw new NullPointerException("Schedule object is null");
		}
	 ...
	 *         conversion fails.
	 */
	public static byte[] scheduleUserToExcelConvert(Schedule schedule,
			Employee user) throws NullPointerException {
		...
		WritableSheet sheet = workbook.createSheet(schedule.getPeriod()
				.getStartDate() + " - " + schedule.getPeriod().getEndDate(), 0);
		...
		try {
			...
			sheet.addCell(label);
			label = new Label(0, 1, "Состояние : " + stateShedule,
					headerTextFormat);
			sheet.addCell(label);

			Map<Date, List<ClubDaySchedule>> listSchedule = schedule
					.getDayScheduleMap();
			...
			List<Club> clubsList = (List<Club>) DAOFactory
					.getDAOFactory(DAOFactory.MSSQL)
					.getClubDAO()
					.getDependentClubsForSchedule(
			...
			calendar.setTime(new Date(listDays.get(listDays.size() - 1)
					.getTime()));
			...
			for (Date curDate : listDays) {
				...
				label = new Label(coll, row++, formatter.format(curDate),
						dateTextFormat);
				sheet.addCell(label);
				label = new Label(coll, row++,
				...
				if (daySchedule != null) {
					...
					for (ClubDaySchedule curScheduleClub : daySchedule) {
						...
						for (Club curClub : clubsList) {
							if (curClub.getTitle().equalsIgnoreCase(
									curScheduleClub.getClub().getTitle())) {
								StringBuffer names = new StringBuffer();

								/*
								 * for (Employee employee :
								 * curScheduleClub.getEmployees()) {
								 */
								Employee employee = null;
								for (int i = 0; i < curScheduleClub
										.getEmployees().size(); i++) {
									employee = curScheduleClub.getEmployees()
											.get(i);
									...
									if (user == null) {
										...
										if (user.getEmployeeId() == employee
												.getEmployeeId()) {
											names.append(user
													.getNameForSchedule());
										}
									}
									if (i < curScheduleClub.getEmployees()
											.size() - 1) {
										names.append("\n");
									}
								}

								label = new Label(coll, row, names.toString(),
										cellTextFormat);
								sheet.addCell(label);
							}
							row++;
	...
	public static void main(String[] args) throws Exception {
		...
		ScheduleDAO daoShedule = DAOFactory.getDAOFactory(DAOFactory.MSSQL)
				.getScheduleDAO();
